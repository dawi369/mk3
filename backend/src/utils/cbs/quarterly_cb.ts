...
class QuarterlyContractBuilder {
  ...
  private getCurrentQuarter(date: Date = new Date()): {
    month: MonthCode;
    year: number;
  } {
  ...
  buildQuarterlyRequest(
    assetClass: PolygonAssetClass,
    eventType: "A" | "AM",
    totalQuarters: number
  ): PolygonWsRequest {
    const currentQuarter = this.getCurrentQuarter();
    const quarters = this.getNextQuarters(
      currentQuarter.month,
      currentQuarter.year,
      totalQuarters
    );
