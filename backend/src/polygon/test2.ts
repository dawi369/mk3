...
if (tickers.hasCode("softs", checkCode)) {
  ...
  console.log(`Code "${checkCode}" not found in softs. Available codes:`, tickers.listCodes("softs"));
}

// Get tickers by sector (useful for us_indices)
const usIndices = tickers.getBySector("us_index");
console.log("US Indices:", usIndices.map(t => `${t.product_code} - ${t.name}`));

// Get tickers by venue
const xcmeTickers = tickers.getByVenue("XCME");
console.log("XCME tickers:", xcmeTickers.map(t => t.product_code));